...
key = 12070
milestone_key = [
    i for i in milestones if milestones[i] == 'JAX Integration Completeness'
]
for k in issues.keys():
  if issues[k].milestone and issues[k].milestone['number'] == milestone_key[0]:
    u.unite(k, key)
...
with open('out.mmd', 'w') as f:
  ...
  for m in milestones.keys():
    ...
    f.write(f'  click m{m} "https://github.com/openxla/iree/milestone/{m}"\n')
